...
void GraphType<VertexType>::BreadthFirstSearch(VertexType startVertex, VertexType endVertex) {
    ...
    distances[IndexIs(vertices, startVertex)] = 0;
    ...
    do {
        ...
        if (vertex == endVertex) {
            cout << vertex << " ";
            found = true;
            cout << "\nShortest path length: " << distances[IndexIs(vertices, vertex)] << endl;
        } else {
            if (!IsMarked(vertex)) {
                MarkVertex(vertex);
                cout << vertex << " ";
                ...
                while (!vertexQ.IsEmpty()) {
                    ...
                    if (!IsMarked(item)) {
                        ...
                        if (distances[itemIndex] == -1) {
                            distances[itemIndex] = distances[vertexIndex] + 1;
                        }
                    }
                }
            }
